...
import org.apache.hadoop.hdfs.server.datanode.fsdataset.impl.FsDataSetBridge;
...
public class MiniDFSCluster {
    ...
    for (MiniDFSNNTopology.NSConf nameservice : nnTopology.getNameservices()) {
      ...
      for (NNConf nn : nameservice.getNNs()) {
        ...
        createNameNode(nnCounter, conf, numDataNodes, false, operation,
            clusterId, nsId, MiniDFSClusterBridge.getNNConf_nnId(nn));
  ...
  private static void initNameNodeAddress(Configuration conf,
    ...
    String key = DFSUtil.addKeySuffixes(
        ...
        MiniDFSClusterBridge.getNNConf_nnId(nnConf));
    ...
    key = DFSUtil.addKeySuffixes(
        ...
        MiniDFSClusterBridge.getNNConf_nnId(nnConf));
    conf.set(key, "127.0.0.1:" + MiniDFSClusterBridge.getNNConf_ipcPort(nnConf));
  }
  ...
  public static boolean changeGenStampOfBlock(int dnIndex, ExtendedBlock blk,
    ...
    File metaFile = FsDataSetBridge.findMetaFile(blockFile);
