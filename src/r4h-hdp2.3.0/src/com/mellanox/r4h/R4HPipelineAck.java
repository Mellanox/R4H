...
public class R4HPipelineAck extends PipelineAck {
	...
	public R4HPipelineAck(long seqno, Status[] replies) {
		super(seqno, toIntArray(replies));
	}
	...
	public R4HPipelineAck(long seqno, Status[] replies, long downstreamAckTimeNanos) {
		super(seqno, toIntArray(replies), downstreamAckTimeNanos);
	}
	...
	private static int[] toIntArray(Status[] statusArray) {
		int[] intArray = new int[statusArray.length];
		for (int i = 0 ; i < statusArray.length ; i++) {
			intArray[i] =  statusArray[i].getNumber();
		}
		return intArray;
	}
}
