...
public class DistributedFileSystem extends FileSystem {
  ...
  public DistributedFileSystem() {
		if (toPrintBreakdown) {
			...
			LOG.info(String.format("%.3f", (float) now / 1000000000.) + ", " + (now - lastOperationTS) / 1000000000.
			        + " : DistributedFileSystem constructed successfully.");
			synchronized (lastOperationTSLock) {
				lastOperationTS = now;
			}
		} else {
			LOG.info("Mellanox RDMA-accelerated DistributedFileSystem constructed successfully.");
		}
  }
  ...
  public DistributedFileSystem(InetSocketAddress namenode,
    ...
    LOG.info("Mellanox RDMA-accelerated DistributedFileSystem constructed.");
  }
...
//	        + " : Before press anykey.");
//    
//    LOG.info("Press any key to continue. Where's the anykey? [ENTER]");
//    System.in.read();
//
//    now = System.nanoTime();
//	LOG.info(String.format("%.3f", (float) now / 1000000000.) + ", " + (now - lastOperationTS) / 1000000000.
//	        + " : Anykey was pressed.");
  ...
  public void close() throws IOException {
    ...
    if (toPrintBreakdown) {
        long now = System.nanoTime();
        LOG.info(String.format("%.3f", (float)now / 1000000000.) + ", " + (now - lastOperationTS) / 1000000000. + " : DistributeFileSystem was closed.");
        synchronized (lastOperationTSLock) {
            lastOperationTS = now;
        }
    }
