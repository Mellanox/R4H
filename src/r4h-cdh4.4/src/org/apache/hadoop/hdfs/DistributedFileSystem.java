...
public class DistributedFileSystem extends FileSystem {
  ...
  private static boolean toPrintBreakdown = true;
  ...
  public DistributedFileSystem() {
		if (toPrintBreakdown) {
			...
			LOG.info(String.format("%.3f", (float) now / 1000000000.) + ", " + (now - lastOperationTS) / 1000000000.
			        + " : Vanilla distributedFileSystem constructed successfully.");
...
//    LOG.info("Vanilla: Press any key to continue. Where's the anykey? [ENTER]");
//    System.in.read();
  ...
  public void close() throws IOException {
    ...
    if (toPrintBreakdown) {
        long now = System.nanoTime();
        LOG.info(String.format("%.3f", (float)now / 1000000000.) + ", " + (now - lastOperationTS) / 1000000000. + " : Vanilla distributeFileSystem was closed.");
        synchronized (lastOperationTSLock) {
            lastOperationTS = now;
        }
    }
