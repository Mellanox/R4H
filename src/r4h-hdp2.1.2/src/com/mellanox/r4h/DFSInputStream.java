...
public class DFSInputStream extends FSInputStream implements ByteBufferReadable, CanSetDropBehind, CanSetReadahead, HasEnhancedByteBufferAccess {
	@VisibleForTesting
	 ...
	 */
	private final IdentityHashStore<ByteBuffer, Object> extendedReadBuffers = new IdentityHashStore<ByteBuffer, Object>(0);
	...
	synchronized void openInfo() throws IOException, UnresolvedLinkException {
		...
		int retriesForLastBlockLength = dfsClient.getConf().getRetryTimesForGetLastBlockLength();
		while (retriesForLastBlockLength > 0) {
			...
			if (lastBlockBeingWrittenLength == -1) {
				...
				waitFor(dfsClient.getConf().getRetryIntervalForGetLastBlockLength());
				lastBlockBeingWrittenLength = fetchLocatedBlocksAndGetLastBlockLength();
			} else {
				break;
			}
	...
	private long readBlockLength(LocatedBlock locatedblock) throws IOException {
		...
		for (DatanodeInfo datanode : locatedblock.getLocations()) {
			...
			try {
				cdp = DFSUtil.createClientDatanodeProtocolProxy(datanode, dfsClient.getConfiguration(), dfsClient.getConf().getSocketTimeout(),
	...
	private synchronized LocatedBlock getBlockAt(long offset, boolean updatePosition) throws IOException {
		...
		if (offset < 0 || offset >= getFileLength()) {
			throw new IOException("offset < 0 || offset >= getFileLength(), offset=" + offset + ", updatePosition=" + updatePosition
	...
	private synchronized DatanodeInfo blockSeekTo(long target) throws IOException {
		...
		while (true) {
			...
			try {
				...
				blockReader = new BlockReaderFactory(dfsClient.getConf()).setInetSocketAddress(targetAddr).setRemotePeerFactory(dfsClient)
				        .setDatanodeInfo(chosenNode).setFileName(src).setBlock(blk).setBlockToken(accessToken).setStartOffset(offsetIntoBlock)
				        .setVerifyChecksum(verifyChecksum).setClientName(dfsClient.clientName).setLength(blk.getNumBytes() - offsetIntoBlock)
				        .setCachingStrategy(cachingStrategy).setAllowShortCircuitLocalReads(!shortCircuitForbidden())
				...
				if (ex instanceof InvalidEncryptionKeyException && refetchEncryptionKey > 0) {
					DFSClient.LOG.info("Will fetch a new encryption key and retry, " + "encryption key was invalid when connecting to " + targetAddr
					        + " : " + ex);
	...
	public synchronized void close() throws IOException {
		...
		if (!extendedReadBuffers.isEmpty()) {
			...
			DFSClient.LOG.warn("closing file " + src + ", but there are still " + "unreleased ByteBuffers allocated by read().  " + "Please release "
			        + builder.toString() + ".");
		}
		if (blockReader != null) {
			blockReader.close();
	 ...
	 * ChecksumFileSystem
	 */
	private synchronized int readBuffer(ReaderStrategy reader, int off, int len, Map<ExtendedBlock, Set<DatanodeInfo>> corruptedBlockMap)
	        throws IOException {
		IOException ioe;
	 ...
	 * @param ignoredNodes
	 *            Do not chose nodes in this array (may be null)
	 * @return The DNAddrPair of the best node.
	 * @throws IOException
	 */
	private DNAddrPair getBestNodeDNAddrPair(final DatanodeInfo[] nodes, Collection<DatanodeInfo> ignoredNodes) throws IOException {
	...
	private static String getBestNodeDNAddrPairErrorString(DatanodeInfo nodes[], AbstractMap<DatanodeInfo, DatanodeInfo> deadNodes,
	...
	private Callable<ByteBuffer> getFromOneDataNode(final DNAddrPair datanode, final LocatedBlock block, final long start, final long end,
	        final ByteBuffer bb, final Map<ExtendedBlock, Set<DatanodeInfo>> corruptedBlockMap, final CountDownLatch latch) {
	...
	private void actualGetFromOneDataNode(final DNAddrPair datanode, LocatedBlock block, final long start, final long end, byte[] buf, int offset,
	        Map<ExtendedBlock, Set<DatanodeInfo>> corruptedBlockMap) throws IOException {
		...
		while (true) {
			...
			try {
				...
				reader = new BlockReaderFactory(dfsClient.getConf()).setInetSocketAddress(targetAddr).setRemotePeerFactory(dfsClient)
				        .setDatanodeInfo(chosenNode).setFileName(src).setBlock(block.getBlock()).setBlockToken(blockToken).setStartOffset(start)
				        .setVerifyChecksum(verifyChecksum).setClientName(dfsClient.clientName).setLength(len).setCachingStrategy(curCachingStrategy)
				        .setAllowShortCircuitLocalReads(allowShortCircuitLocalReads).setClientCacheContext(dfsClient.getClientContext())
				        .setUserGroupInformation(dfsClient.ugi).setConfiguration(dfsClient.getConfiguration()).build();
				...
				String msg = "fetchBlockByteRange(). Got a checksum exception for " + src + " at " + block.getBlock() + ":" + e.getPos() + " from "
				        + chosenNode;
		...
		while (true) {
			...
			if (futures.isEmpty()) {
				...
				try {
					...
					if (DFSClient.LOG.isDebugEnabled()) {
						DFSClient.LOG.debug("Waited " + dfsClient.getHedgedReadTimeout() + "ms to read from " + chosenNode.info
	...
	private Future<ByteBuffer> getHedgedReadFuture(final DNAddrPair chosenNode, final LocatedBlock block, long start, final long end,
	        final ByteBuffer bb, final Map<ExtendedBlock, Set<DatanodeInfo>> corruptedBlockMap, final CountDownLatch hasReceivedResult) {
		Callable<ByteBuffer> getFromDataNodeCallable = getFromOneDataNode(chosenNode, block, start, end, bb, corruptedBlockMap, hasReceivedResult);
		return dfsClient.getHedgedReadsThreadPool().submit(getFromDataNodeCallable);
	}

	private ByteBuffer getFirstToComplete(ArrayList<Future<ByteBuffer>> futures, CountDownLatch latch) throws ExecutionException,
		...
		throw new InterruptedException("latch has counted down to zero but no" + "result available yet, for safety try to request another one from"
		...
		if (pos <= targetPos && targetPos <= blockEnd) {
			...
			if (diff <= blockReader.available()) {
				try {
					...
					if (DFSClient.LOG.isDebugEnabled()) {
						DFSClient.LOG.debug("Exception while seek to " + targetPos + " from " + getCurrentBlock() + " of " + src + " from "
						        + currentNode, e);
					}
				}
			}
   ...
   */
	 ...
	 */
	static DatanodeInfo bestNode(DatanodeInfo nodes[], AbstractMap<DatanodeInfo, DatanodeInfo> deadNodes, Collection<DatanodeInfo> ignoredNodes)
	        throws IOException {
		if (nodes != null) {
	...
	@Override
	public synchronized ByteBuffer read(ByteBufferPool bufferPool, int maxLength, EnumSet<ReadOption> opts) throws IOException,
		...
		if (dfsClient.getConf().isShortCircuitMmapEnabled()) {
	...
	private synchronized ByteBuffer tryReadZeroCopy(int maxLength, EnumSet<ReadOption> opts) throws IOException {
		...
		if ((curPos + maxLength) <= (curEnd + 1)) {
			...
			if (length63 <= 0) {
				if (DFSClient.LOG.isDebugEnabled()) {
					DFSClient.LOG.debug("Unable to perform a zero-copy read from offset " + curPos + " of " + src + "; " + length63
					        + " bytes left in block.  " + "blockPos=" + blockPos + "; curPos=" + curPos + "; curEnd=" + curEnd);
				}
				return null;
			}
			if (DFSClient.LOG.isDebugEnabled()) {
				DFSClient.LOG.debug("Reducing read length from " + maxLength + " to " + length63 + " to avoid going more than one byte "
				        + "past the end of the block.  blockPos=" + blockPos + "; curPos=" + curPos + "; curEnd=" + curEnd);
			}
		}
			...
			if (length31 <= 0) {
				...
				if (DFSClient.LOG.isDebugEnabled()) {
					DFSClient.LOG.debug("Unable to perform a zero-copy read from offset " + curPos + " of " + src
					        + "; 31-bit MappedByteBuffer limit " + "exceeded.  blockPos=" + blockPos + ", curEnd=" + curEnd);
				}
				return null;
			}
			length = (int) length31;
			if (DFSClient.LOG.isDebugEnabled()) {
				DFSClient.LOG.debug("Reducing read length from " + maxLength + " to " + length + " to avoid 31-bit limit.  " + "blockPos=" + blockPos
		...
		try {
			...
			if (DFSClient.LOG.isDebugEnabled()) {
				DFSClient.LOG.debug("readZeroCopy read " + length + " bytes from offset " + curPos + " via the zero-copy read "
